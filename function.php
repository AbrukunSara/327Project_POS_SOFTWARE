...
function db_connect(){
   $connection=mysqli_connect("localhost", "root", "", "dorypos");
  return $connection;
}
if (!db_connect()) {
...
function brand_picker($db){
  $brand="SELECT * FROM dorypos_brand ";
  $brand_sql=mysqli_query($db,$brand);
  return $brand_sql;
}
function vendor_picker($db){
  $vendor="SELECT * FROM dorypos_vendor ORDER BY vendor_id desc";
  $vendor_sql=mysqli_query($db,$vendor);
  return $vendor_sql;
}

function Category_picker($db){
  $category="SELECT * FROM dorypos_category";
...
function product_checker($db,$barcode){
 $check="SELECT * FROM dorypos_product WHERE barcode='{$barcode}' ";
 $check_result=mysqli_query($db,$check);
 return $check_result;
}
function product_picker($db){
 $check="SELECT * FROM dorypos_product ";
 $check_result=mysqli_query($db,$check);
 return $check_result;
}
